...
NOOP is a standalone, fully **offline** companion app for WHOOP 5.0 and MG straps. It pairs
...
no cloud, no account. A WHOOP 4.0 strap is recognised on the air and reported as
detected-but-unsupported; NOOP does not connect to it. This document explains how the repository is
laid out, how to build and test it, the conventions every change is expected to follow, and the
safety rules that are non-negotiable (especially on the Bluetooth path).
...
1. **Offline by design.** There is no NOOP server, telemetry, or account, and **nothing about you
   ...
   Every network request the app can make is documented in
   [docs/PRIVACY_SECURITY.md §1.1](PRIVACY_SECURITY.md) — the opt-in AI Coach and the update check
   (a read of a public version number, on by default, switchable off) are the standing examples.
   Adding another needs a very good reason and the same treatment: named in the privacy doc, and
   switchable off. New hosted services or undisclosed network calls do not belong here; see
   [Scope](SCOPE.md).
...
5. **Credit upstream.** The protocol work is built on prior community reverse-engineering —
   `johnmiddleton12/my-whoop` (the original WHOOP 4.0 work the framing and store model descend from)
   and `b-nnett/goose` (WHOOP 5.0 / MG). Preserve those credits in code comments and in
   [`../ATTRIBUTION.md`](../ATTRIBUTION.md).
...
The codebase is split into reusable Swift packages plus a thin app layer per Apple platform. The
**macOS app is the reference implementation**; **iOS was folded into `main` in v1.94** and is a
**build-from-source / sideload target** (`NOOPiOS` / `NOOPiOSWidgets`) — no App Store/TestFlight, to
keep the project anonymous (see [`IOS.md`](IOS.md)). Both reuse the same packages.
...
├── Strand/                     # macOS SwiftUI app target (product name: NOOP); shared with iOS
...
├── StrandiOS/                  # iOS app shell (StrandiOSApp, RootTabView, HealthKit bridge)
├── StrandiOSShared/            # code shared between the iOS app and its extensions
...
│   ├── StrandImport/           # WHOOP CSV, Apple Health, FIT/GPX/TCX and related importers
...
| CoreBluetooth, bonding, offload, live state | `Strand/BLE`, `Strand/Collect` | App layer (shared by macOS and iOS) — wraps the pure packages. |
| A screen, sidebar item, menu-bar UI, automation | `Strand/Screens`, `Strand/App`, `Strand/System` | App layer. |
| An iOS-only surface (tab shell, widgets, HealthKit) | `StrandiOS/`, `StrandiOSShared/`, `StrandiOSWidgets/` | iOS app layer; see [`IOS.md`](IOS.md). |
...
import, and design layers compile and run unmodified under both app targets. Any
...
that is what breaks the package contract (and the Linux builds of the pure packages). CoreBluetooth
lives only in the app layer's `Strand/BLE`.
...
- **On every PR (required):** `source-hygiene`, `tools-python` and `i18n-coverage` have no path
  filter, so all three run on everything. `swift-packages` (`swift test` for `Packages/**` plus a
  build of the Swift tools under `Tools/`) is **path-filtered** — it runs when you touch what it
  covers, which is most substantive PRs. Between them these catch the regressions that matter most
  (protocol/analytics math, storage, i18n) without a device or an app build. The check names you see
  are JOB names and do not resemble the workflow names; the table in the root
...
- **Disabled by design — you build the app yourself:** `app-build.yml` (app-target compile, iOS needs
  ...
  `Repository`, `RootTabView`) passes every default check — nothing in the default set compiles the
  app targets. Before you push app-layer changes, compile locally (`xcodebuild … build` for the
  `Strand` scheme, and the `NOOPiOS` scheme if you touched a shared or iOS file) or dispatch
  `app-build.yml` on demand.
...
- **Absent on purpose:** dependency/vuln scanning and any device-connected test. The dependency set
  is small and pinned, there is no server or telemetry, and BLE/offload behavior is validated **on a
  real strap** — compile-success proves nothing about connection behavior.
...
- **Outbound:** `WhoopCommand.frame(seq:payload:)` builds the `0xAA`-led envelope around
  `[type=35][seq][cmd][payload…]` with the header check and the zlib `crc32` trailer over the inner
  bytes. The WHOOP 5.0 / MG envelope (the "goose" path) carries a **CRC16-Modbus** header check —
  see `Packages/WhoopProtocol/Sources/WhoopProtocol/Framing.swift` (`verifyFrame(_:family:)`,
  `crc16Modbus`, `crc32`). The older WHOOP 4.0 CRC8 envelope is documented in
  [`PROTOCOL_WHOOP4.md`](PROTOCOL_WHOOP4.md) as a protocol reference only; NOOP no longer implements
  it, and a 4.0 strap is reported as detected-but-unsupported rather than connected.
...
4. **Send it through the existing path** — `BLEManager.send(_:payload:writeType:)` — which frames the
   command (correct header check + CRC32 trailer) and writes to the command characteristic. Don't
   build raw writes by hand.
...
`DatabaseMigrator`** (currently through `v46`).
...
- **Update `schema_oracle.json` in the same PR.** The resulting schema is pinned by a fixture
  (`Packages/WhoopStore/Tests/WhoopStoreTests/Resources/schema_oracle.json`) that
  `SchemaOracleTests.swift` compares to GRDB's `PRAGMA table_info`. It fails on an unpinned column, a
  column ORDER difference, a type/nullability/DEFAULT change, a primary-key change, an index change,
  or a new unpinned table — so a migration cannot land without its schema being written down. A
  divergence that is deliberate must be written into the fixture's `divergenceReasons` with the reason
  and what closing it would cost; the suite also fails on a ledger entry that has stopped being true,
  so the list can only shrink on purpose. Extend the oracle rather than adding a parallel mechanism
  (same idiom as `decoder_oracle.json`).
...
- **Each package owns its tests** under `Packages/<Name>/Tests/…`; run them with `swift test`.
  Coverage already includes framing/CRC, reassembly, schema, stream decode, store
...
- **Sample exports live with the tests.** `StrandImport` test resources (sample WHOOP CSVs, lift
  program sheets, journal exports) sit under `Packages/StrandImport/Tests/StrandImportTests/` and are
  bundled via the package's `Package.swift`.
- **The golden decoder oracle is where decode behaviour is pinned.** `decoder_oracle.json`
  (`Packages/WhoopProtocol/Tests/WhoopProtocolTests/Resources/`) is what `DecoderOracleTests.swift`
  asserts against: decoded field VALUES per fixture frame, and the assembled `Streams`/`StreamBatch`
  shape (per-stream row counts + the emptiness verdict) per fixture batch. Pinning values rather than
  bytes is the point — a 32-vs-64-bit or signedness regression is invisible to a fixture-hex test.
  **Extend the oracle rather than adding a parallel mechanism**; a `coverage` manifest in the file
  makes silently dropping an assertion a test failure, so adding one means listing it there too.
- **Frame integrity is pinned by the same idiom.** Whether a frame is *intact* is not a matter of
  reading `Framing.swift` — it is pinned by `frame_integrity_oracle.json`, which lists, per frame, the
  verdict, the reject reason **and** the historical-metadata classification. No generator is retained
  in this repository: update the fixture, then run `FrameIntegrityOracleTests.swift`, which asserts
  every row and all three fields. Changing a bound, a reason or a classification without updating the
  fixture fails the suite. Extend that oracle rather than adding a test that can only drift.
...
in `project.yml` (`MARKETING_VERSION`); the build number (`CFBundleVersion`) increments independently
on every release.
...
NOOP's logic already lives in the shared packages, so most platform work is app-layer wiring rather
than rewrites of the core. Today the **macOS app is the working reference implementation** and
**iOS is a sideload / build-from-source target**; the items below are planned, experimental, or
deferred. Contributions toward these are welcome — open an issue to coordinate first.
...
- **iOS (sideload / build-from-source target on `main`).** iOS was folded into `main` in v1.94 as a
  first-class target — the `NOOPiOS` and `NOOPiOSWidgets` schemes (app target plus widgets, a Live
  Activity, and HealthKit), built against current code in Xcode; `app-build.yml` compiles both macOS
  and iOS when dispatched. It is **not on the App Store, intentionally:** the App Store and TestFlight
  both require a real Apple Developer identity, which is at odds with NOOP staying anonymous, so the
  release `.ipa` is unsigned and sideloaded with the user's own Apple ID. Every package declares
  `.iOS(.v16)` and guards UI-framework code with `#if canImport(UIKit)/AppKit`, so the shared core and
  analytics run unmodified — results match macOS. It is newer and less battle-tested than macOS (live
  BLE on a real iPhone is still being validated), so on-hardware testing is especially welcome;
  [`IOS.md`](IOS.md) is the detailed guide.

...
| W02-005 | S2 | Fixed | "Delete all of this device's data" and "Forget device" leave the device's computed `<id>-noop` rows (scored days, sleeps, workouts, metric series) on disk and on screen | `Strand/Data/DeviceRegistry.swift` `deleteDeviceData`, `forget`; `DeviceRegistryStore.swift` `deleteAllData` | P5 confirmed. V0: `DeviceRegistryStoreTests.testDeleteAllDataAlsoClearsTheComputedSibling` failed (the sibling row survived). Fix: `deleteAllData` also clears `<id>-noop`. V1: passes; fails with the fix reverted; WhoopStore suite green. Multi-strap remainder is W07-002 |  |

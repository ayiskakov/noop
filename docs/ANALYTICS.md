...
`Tools/SleepTrain/sleeptrain/features.py` is the feature definition. The Swift port is pinned to it by `SleepStagerV3OracleTests`, which rebuilds three synthetic nights from a seeded generator and requires every feature and posterior within 1e-9 of the reference and every label equal. `Tools/SleepPSG --section v3` replays both shipped stagers over the prepared cohorts. On DREAMT it reproduces the Python figures exactly.

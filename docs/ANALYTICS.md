...
- App reference implementation: `Strand/` (SwiftUI, macOS + iOS). The same `StrandAnalytics` code backs both app targets — the number-crunching is shared, so results match across macOS and iOS.

---

## What is actually wired into the app

The package contains more analytics than the app currently surfaces. This section is the honest map of **library-only** vs **live**, verified against the app sources. The status below is shared across the two app targets (macOS + iOS).
...
effects of prior sleep restriction disappeared?”. The constants, recurrence, deadband,
nap credit, and 14-usable-night bound are pinned by golden-vector tests.
...
> **Honest hedging.** These stages are **approximations**, not medical advice. The default recipe (V3, below) is measured against polysomnography on other wrist devices, never on a WHOOP strap: no public dataset pairs one with PSG. The EEG-free 4-class ceiling is ~65–73% epoch agreement (Walch 2019). **Light/deep separation is the weakest link — deep-minute estimates are the least reliable output.**

Three recipes can stage a detected night, chosen in Settings → Experimental → Sleep staging (`PuffinExperiment.sleepStager`, a `SleepStagerVersion`): **V3** (`SleepStagerV3`, the default), **V2** (`SleepStagerV2`, the default before V3) and **V1** (`SleepStager.stageSession`, stages 1–3 below). Detection (stage 0) is the same for all three. A user who had switched the old V2 toggle off stays on V1.
...
### `SleepStagerV3` — the default: a stager fitted to polysomnography

V1 and V2 set every coefficient by hand. Every published wrist stager that reaches a four-class kappa of 0.5 fits its per-epoch emissions to human-scored PSG instead (Fitbit, Oura, Philips, Fonseca et al. 2023, Walch et al. 2019). V3 does the same, on open data only, and is fitted and reported by `Tools/SleepTrain`:

- **Features, all relative to the night itself.** Motion is per-second gravity jerk over the night's median jerk (V2's self-calibrating floor), with time since and until the last movement; posture change is the change of the wrist's z-angle between 5 s blocks over the night's median change. Heart rate enters as within-night z-scores, percentile ranks and 5 / 11 / 20-minute variability. When the strap delivers beat intervals, 5-minute HRV (RMSSD, SDNN, pNN50, LF and HF power, LF/HF, the HF peak as a breathing rate, its peakedness, DFA α1) is scaled to the night's 5th–95th percentiles. Elapsed time since the start of the staged span enters both as minutes and as three decays, `exp(−t/10)`, `exp(−t/30)` and `exp(−t/90)`. A linear head reads minutes only as a straight line: before the decays it staged deep within 5 min of onset on 37 of 88 Wearanize+ nights, against none in the PSG. An absolute-angle posture feature was tried first and failed across devices, because a WHOOP strap reads about seven times the angle change of an Apple Watch on still sleep.
- **Two heads.** Multinomial logistic regressions turn the features into stage posteriors: the base head reads motion and heart rate, the HRV head the beat features as well. When at least half the staged epochs have 20 or more beats in their 5-minute window, both run and their log-posteriors are averaged with equal weight. Otherwise the base head stages the night alone. The HRV head alone scored 0.554 on Wearanize+ and 0.320 on DREAMT against the blend's 0.547 and 0.313, but it called REM high out of its training cohort (+7.9 points on DREAMT, a quarter to over a third of sleep on a WHOOP night), and the blend cuts that to +5.9. The weights are in the generated `SleepStagerV3Model.swift`.
- **Decoder.** Viterbi over `log P − 0.5 · log prior`, with the stage-transition matrix and start distribution counted from the training hypnograms.
- **Band window.** With `SleepStager.bandSleepWindow` present, V3 stages only the epochs inside it (features, normalisation and decoding all run over that span) and every other epoch is wake. Training used every night twice, over its whole scored span and cropped by the same window rule applied to its PSG hypnogram, so both cases are what the model was fitted on.
- **Missing channels.** A gap in a feature takes the night's median of it. A feature missing for the whole span takes the head's training mean, so it standardises to 0 and moves no logit; it used to take 0, which for a percentile rank is the night's extreme, and a span with no heart rate staged a third to a half of itself deep. A span with no motion measurement at all (no gravity, or none in two consecutive seconds) has its motion and posture features missing, not read as the stillest night the model ever saw. No night V3 was trained or tested on has either gap, so neither rule moves a figure below.
- **Nights with no motion.** `SleepStager.hrOnlySessions` stages a night the strap banked no gravity for with the user's choice too (V1, which cannot stage without motion, hands it to V2). With the motion stripped from the PSG nights, V3 scores four-class kappa 0.477 / 0.446 / 0.275 in the window on Wearanize+ / sleep-accel / DREAMT against V2's 0.303 / 0.363 / 0.224, though its first REM comes 40–74 min late there. Only DREAMT is out of sample; V3 never saw any night without motion.

Training data: Wearanize+ (88 nights, Empatica E4 wristband, CC BY 4.0) and PhysioNet sleep-accel (31 nights, Apple Watch, ODC-By). Agreement with PSG, cross-validated 10-fold by subject, every scored epoch counted and wake outside the window:

| Cohort | V2 | V3 | Second human scorer |
|---|---|---|---|
| Wearanize+, in the band-style window | 0.363 | 0.547 | 0.742 |
| Wearanize+, whole span | 0.301 | 0.533 | — |
| sleep-accel, in the band-style window (no beat intervals) | 0.434 | 0.504 | — |
| sleep-accel, whole span | 0.373 | 0.463 | — |
| DREAMT, 10 sleep-clinic patients never trained on, in the window | 0.259 | 0.313 | — |

On Wearanize+ (in the window) V3's pooled stage shares sit within 1 percentage point of PSG, where V2 over-counts light by 13.5 points and under-counts deep and REM by 5 and 7. First deep comes 6 min early on average, and never within 5 min of onset. First REM comes 47 min late, as V2's does on this cohort (48), and 60 min late on sleep-accel, where V2 is close (5). Clinic nights are harder for every recipe: long still wake and little deep sleep, which V3 over-counts by about 50 min a night. DREAMT's licence limits it to research, so it only ever tests the model and never enters the fit.

`Tools/SleepTrain/sleeptrain/features.py` is the feature definition. The Swift port is pinned to it by `SleepStagerV3OracleTests`, which rebuilds four synthetic nights (one of them without gravity) from a seeded generator and requires every feature and posterior within 1e-9 of the reference and every label equal. `Tools/SleepPSG --section v3` replays both shipped stagers over the prepared cohorts. On DREAMT it reproduces the Python figures exactly.
...
- **`SleepStagerV2`.** On a motion-quiescent epoch (no observed movement; peak jerk at/below the night-relative wake-gate floor) the AWAKE cardiac term is clamped to **≤ 0** — wake-*suppressing* (low, flat HR) evidence is kept, the wake-*promoting* half is dropped. A still low-HR epoch is byte-identical; genuine motion and the night-relative jerk gate still drive wake.
...
`SleepStagerV2` takes the span the trim keeps (`SleepStager.bandSleepWindow`) as its `sleepWindow` and forces wake outside it inside the Viterbi lattice, rather than having the trim relabel its output afterwards. `SleepStagerV3` takes the same window and stages only inside it. The trim still runs after it and finds nothing left to change. V1 has no such input and relies on the trim alone. This matters because the recipe measures its REM-latency guard from the onset its own first pass finds. When the trim came afterwards, that onset was the recipe's own, up to two hours before the band's on a long lead-in, so the guard was spent before sleep began: on one 5/MG night REM appeared 31 min after the band's onset. On PhysioNet sleep-accel, with each PSG hypnogram standing in for the band (31 subjects), the lattice form scores four-class kappa 0.414 against 0.413 for relabelling afterwards, and first-REM latency error 44.0 min against 47.4. Without a band it is inert. Only the onset guards move to the window: V2's cycle prior (deep early, REM rising toward morning) still reads the fraction of the whole session, so a long lead-in spends part of its early deep prior before sleep begins. Measuring that fraction over the window instead scored no better against PSG (kappa 0.363 → 0.360 on Wearanize+, 0.434 → 0.427 on sleep-accel, 0.259 → 0.261 on DREAMT) and moved first REM later on all three, so the session clock stays.
...
### V2 deep-latency guard — no N3 at sleep onset

Descent into N3 runs through N1 and N2, so the first deep epoch comes minutes after sleep onset. The recipe's deep terms, the HR-flatness gate and the early-night cycle prior, are strongest at onset itself. So `SleepStagerV2` staged deep at the first sleep epoch: on 6 of 30 PhysioNet sleep-accel subjects within 5 min of onset (1 of 30 in the PSG truth), 12 of 30 once a band window fixes the onset, and at minute 0 on every one of six banked 5/MG nights. The deep emission now carries a penalty of 3.0 log-odds at onset, decaying linearly to zero 20 min after it. It is the twin of the REM-latency guard, with the same magnitude, measured from the same onset and applied in the same second pass. It holds deep back rather than forbidding it.

The magnitude is the REM guard's. The 20 min decay sits inside the classic first-cycle descent, and on sleep-accel it is where staged first-deep latency is unbiased against PSG (bias −3.3 min, against −12.5 without the guard). Kappa rises across the whole 10–25 min grid, so the value was chosen for the unbiased latency and not for the kappa a longer guard would buy. Shipped result on sleep-accel: kappa 0.363 → 0.373, deep F1 0.498 → 0.523, pooled deep bias +5.17 → +3.57 pp, with REM and wake unchanged. On the six 5/MG nights, first deep moves from minute 0 to 10–28 min after onset, deep falls 1–3 points to 15–27 % of sleep, and TST does not move. `Variants.deepGuardOff` in `Tools/SleepPSG` reports the recipe without it.
...
- **Not affiliated with WHOOP.** NOOP interoperates with hardware and exports you already own, entirely on-device. Protocol decoding builds on community reverse-engineering of the WHOOP BLE protocol (project *my-whoop*, `johnmiddleton12/my-whoop`, for the original 4.0-generation work whose conventions carried over) and the WHOOP 5.0 / MG protocol (project *goose*, `b-nnett/goose`).

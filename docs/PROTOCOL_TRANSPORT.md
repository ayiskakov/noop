...
Frames built by the rule above are longer than 15 bytes, with `(declared length − 4)` divisible by four, and every captured frame so far has that shape. NOOP's acceptance is looser on both counts and relies on the checksums instead: it requires exact agreement between declared and supplied length, a frame of at least 13 bytes (`FrameLimits.whoop5MinimumFrameBytes`, an empirical floor that keeps the inner type byte; see [the reassembler notes](PROTOCOL_IMPLEMENTATION.md)), a matching header CRC16 and a matching body CRC32, and it does not check alignment. Do not require one constant value for bytes 4–5. Header CRC is reflected Modbus CRC16, polynomial `0xA001`, initial value `0xFFFF`. Format-1 bodies use the standard reflected CRC32 convention, polynomial `EDB88320h`, initial and final XOR `FFFFFFFFh`, including padding.

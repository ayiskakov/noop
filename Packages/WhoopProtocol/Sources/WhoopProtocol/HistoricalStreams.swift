...
public func rejectedHistoricalRecords(_ rawFrames: [[UInt8]], family: DeviceFamily) -> [[UInt8]] {
    ...
    return rawFrames.filter { f in
        ...
        // v16 MAX86176 FIFO (#891): skipped on the SAME verdict-bound terms as v26 above, and for the same
        // reason — `extractHistoricalStreams` stores an intact v16 record's FIFO durably in its own stream
        // (`Streams.ecgCandidate` / WhoopStore's `ecgCandidateSample`). The skip holds only while that
        // premise does: a REJECTED v16 record is dropped by that extraction like any other, so archive it.

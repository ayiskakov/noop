...
public enum RRSourceChannel: Int, Equatable, Codable, Sendable, CaseIterable {
    ...
    /// Raw values 1-4 are RETIRED. They labelled the multi-tag optical channels of a non-WHOOP source
    /// that NOOP no longer supports, and are deliberately left unclaimed so a row written under the old
    /// code is never silently reinterpreted as a WHOOP transport. Reads decode an unknown code to nil.
...
public struct RRInterval: Equatable, Codable {
    public let ts: Int          // wall-clock unix seconds
    public let rrMs: Int
    /// The transport this beat arrived on, or nil when the source does not distinguish one (legacy
    /// unlabelled rows, and rows written before the column existed). See `RRSourceChannel`.
...
/// Raw skin-temperature register → °C.
///
/// WHOOP 5/MG banks CENTI-DEGREES: the same `/100` scale the 5/MG decoder already used, verified on
/// real captures at both ends of the range (worn 3060 = 30.6 °C, off-wrist 2247 = 22.5 °C). `family`
/// is carried so the call site still states WHICH hardware a reading came from rather than assuming it.
